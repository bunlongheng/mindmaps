...
describe('SidePanel - lock toggle', () => {
  it('delete is enabled and the lock reads Lock map when the map is unlocked', () => {
    loadDiagram()
    render(<SidePanel nodeId={null} onClose={vi.fn()} onDelete={vi.fn()} />)
    fireEvent.click(screen.getByText('Share'))
    expect(screen.getByLabelText('Delete map')).not.toBeDisabled()
    expect(screen.getByLabelText('Lock map')).toBeInTheDocument()
    expect(screen.getByText(/Lock the map to stop accidental edits/)).toBeInTheDocument()
  })

  it('delete is disabled and the lock reads Unlock map when the map is locked', () => {
    loadDiagram(makeDiagram({ locked: true }))
    render(<SidePanel nodeId={null} onClose={vi.fn()} onDelete={vi.fn()} />)
    fireEvent.click(screen.getByText('Share'))
    const del = screen.getByLabelText('Delete map')
    expect(del).toBeDisabled()
    expect(del.getAttribute('title')).toMatch(/locked/i)
    expect(screen.getByLabelText('Unlock map')).toBeInTheDocument()
    // Copy states it plainly: an accident guard, not a password.
    expect(screen.getByText(/accident guard, not a password/)).toBeInTheDocument()
  })

  it('a disabled delete never opens the confirm dialog', () => {
    loadDiagram(makeDiagram({ locked: true }))
    render(<SidePanel nodeId={null} onClose={vi.fn()} onDelete={vi.fn()} />)
    fireEvent.click(screen.getByText('Share'))
    fireEvent.click(screen.getByLabelText('Delete map'))
    expect(screen.queryByText('Delete map?')).not.toBeInTheDocument()
  })

  it('clicking the lock button flips the flag through the store', async () => {
    ;(fetch as unknown as ReturnType<typeof vi.fn>).mockResolvedValue({ ok: true } as Response)
    loadDiagram()
    render(<SidePanel nodeId={null} onClose={vi.fn()} onDelete={vi.fn()} />)
    fireEvent.click(screen.getByText('Share'))
    await act(async () => { fireEvent.click(screen.getByLabelText('Lock map')) })
    await waitFor(() => expect(useMindmapStore.getState().activeMindmap!.locked).toBe(true))
    expect(screen.getByLabelText('Delete map')).toBeDisabled()
  })
})
